...
void TvChannelManager::InitializeWithObjects(jobject managerObject)
{
    ...
    mGetTvChannelListMethod = env -> GetMethodID(managerClass, "getTVChannelList", "()[Lcom/tcl/chip/tvapp/ChannelInfo;");
    if (mGetTvChannelListMethod == nullptr)
    {
        ChipLogError(Zcl, "Failed to access TvChannelManager 'getTVChannelList' method");
        env->ExceptionClear();
    }

    mGetTvChannelLineupMethod = env -> GetMethodID(managerClass, "getTVChannelLineup", "()Lcom/tcl/chip/tvapp/ChannelLineupInfo;");
    if (mGetTvChannelLineupMethod == nullptr)
    {
        ChipLogError(Zcl, "Failed to access TvChannelManager 'getTVChannelLineup' method");
        env->ExceptionClear();
    }

    mGetCurrentTvChannelMethod = env -> GetMethodID(managerClass, "getCurrentTVChannel", "()Lcom/tcl/chip/tvapp/ChannelInfo;");
    if (mGetCurrentTvChannelMethod == nullptr)
    {
        ChipLogError(Zcl, "Failed to access TvChannelManager 'getCurrentTVChannel' method");
        env->ExceptionClear();
    }

    mChangeChannelMethod = env->GetMethodID(managerClass, "changeChannel",
                                            "(Ljava/lang/String;)Lcom/tcl/chip/tvapp/ChannelInfo;");
...
CHIP_ERROR TvChannelManager::getTvChannelList(chip::app::AttributeValueEncoder & aEncoder)
{
    ...
    VerifyOrExit(mGetTvChannelListMethod != nullptr, err = CHIP_ERROR_INCORRECT_STATE);
    VerifyOrExit(env != NULL, err = CHIP_JNI_ERROR_NO_ENV);

    return aEncoder.EncodeList([env, this](const chip::app::TagBoundEncoder & encoder) -> CHIP_ERROR {
        jobjectArray channelInfoList = (jobjectArray) env -> CallObjectMethod(mTvChannelManagerObject, mGetTvChannelListMethod);
        ...
        for (jint i = 0; i < length; i++) {
            ...
            jfieldID getCallSignField = env->GetFieldID(
                channelClass, "callSign", "Ljava/lang/String;");
            ...
            jfieldID getNameField = env->GetFieldID(
                channelClass, "name", "Ljava/lang/String;");
            ...
            jfieldID getJaffiliateCallSignField = env->GetFieldID(
                channelClass, "affiliateCallSign", "Ljava/lang/String;");
...
CHIP_ERROR TvChannelManager::getTvChannelLineup(chip::app::AttributeValueEncoder & aEncoder)
{
    ...
    VerifyOrExit(mTvChannelManagerObject != nullptr, err = CHIP_ERROR_INCORRECT_STATE);
    VerifyOrExit(mGetTvChannelLineupMethod != nullptr, err = CHIP_ERROR_INCORRECT_STATE);
    VerifyOrExit(env != NULL, err = CHIP_JNI_ERROR_NO_ENV);

    {
        chip::app::Clusters::TvChannel::Structs::TvChannelLineupInfo::Type channelLineupInfo;

        jobject channelLineupObject = env->CallObjectMethod(mTvChannelManagerObject, mGetTvChannelLineupMethod);
...
CHIP_ERROR TvChannelManager::getCurrentTvChannel(chip::app::AttributeValueEncoder & aEncoder)
{
...

    {
        chip::app::Clusters::TvChannel::Structs::TvChannelInfo::Type channelInfo;
            ...
            jobject channelInfoObject = env->CallObjectMethod(mTvChannelManagerObject, mGetTvChannelListMethod);
...
bool TvChannelManager::changeChannelByNumber(uint16_t majorNumber, uint16_t minorNumber){
    ...
    ChipLogProgress(Zcl, "Received TvChannelManager::tvChannelClusterChangeChannelByNumber majorNumber %d, minorNumber %d", majorNumber, minorNumber);
    VerifyOrExit( mTvChannelManagerObject != nullptr, ChipLogError(Zcl, "mTvChannelManagerObject null"));
    ...
    ret = env->CallBooleanMethod(mTvChannelManagerObject, mchangeChannelByNumberMethod, static_cast<jint>(majorNumber),static_cast<jint>(minorNumber));
    if (env->ExceptionCheck())
    {
